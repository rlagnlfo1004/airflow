...
with DAG(
    ...
    def check_api_update(http_conn_id, endpoint, base_dt_col, **kwargs):
        ...
        url = f'http://{connection.host}:{connection.port}/{endpoint}/1/1000'
        ...
        try:
            pendulum.from_format(last_date, 'YYY-MM-DD')
        except:
            from airflow.exception import AirflowException
            AirflowException(f'last_date: {base_dt_col} 컬럼은 YYYY.MM.DD 또는 YYYY/MM/DD 형태가 아닙니다.')
