...
class CustomPostgresHook(BaseHook):
    ...
    def get_conn(self):
        ...
        self.postgres_conn = psycopg2.conntect(host=self.host, dbname=self.dbname, user=self.user, password=self.password, port=self.port)
    ...
    def bulk_load(self, table_name, file_name, delimiter: str, is_header: bool, is_replace: bool):
        ...
        file_df = pd.read_csv(file_name, delimiter=delimiter, header=header)

        for col in file_df.colums:
            try:
